...
(later) **Discord** through one API and one CLI. Run `chatensemble` with no
...
One YAML file per account in `~/chatensemble/accounts/` (git-ignored; override with
`--accounts-dir`). Name = filename; `type` from `type:` or the filename / token
prefix. Values expand `${VAR}` / `$VAR` from the environment. `chatensemble accounts`
lists what's configured.

**Mattermost** — `~/chatensemble/accounts/work.yaml`
...
**Slack, app token** — `~/chatensemble/accounts/slack.yaml`
...
```sh
uv run chatensemble import-slack --list              # signed-in workspaces (no secrets)
uv run chatensemble import-slack -w ellis -a ellis   # -> ~/chatensemble/accounts/ellis.yaml
```
...
```sh
uv run chatensemble accounts                       # list configured accounts
uv run chatensemble channels --account work        # channels + unread counts

uv run chatensemble unread                         # unread, dense table
uv run chatensemble unread --feed                  # unread, feed of cards
uv run chatensemble fetch --since 7d --channel 'eng-*' --query 'deploy|incident'
uv run chatensemble fetch --since 3d --feed --lines 2 --width 90

uv run chatensemble fetch --backfill               # pull all history (see Caching)
uv run chatensemble cache                          # show what's cached
```

### Browse threads

With **no subcommand**, `chatensemble` fetches your real accounts and opens an
interactive, `rich`-drawn thread browser:

```sh
uv run chatensemble                                # fetch (last 21 days), then browse
uv run chatensemble --days 30 --account work       # narrower window / one account
uv run chatensemble --no-fetch                     # straight from the cache, no network
uv run chatensemble --synthetic                    # generated demo data instead
```
...
`--synthetic` uses `chatensemble.synthetic.synthetic_messages()` — deterministic
...
`~/chatensemble/cache/_read_overlay.json` (thread ids), re-applied on every load
...
with `chatensemble cache --clear`. Piping the output (no TTY) prints a
per-account feed instead.

From Python:

```python
from chatensemble import ChatManager, FetchFilter, print_feed

with ChatManager.from_dir() as mgr:          # ~/chatensemble/accounts
...
`chatensemble install-skill` drops a Claude Code skill at `~/.claude/skills/chatensemble/`
...
```sh
uv run chatensemble install-skill               # -> ~/.claude/skills/chatensemble/
uv run chatensemble fetch --since 30d --quiet   # refresh the cache; one line per account, no message dump
```

The skill queries `~/chatensemble/cache/*/*.parquet` with DuckDB (via `uv run --with
duckdb`, no install needed), refreshes first with `fetch --quiet` when data may be
stale, and excludes threads listed in `~/chatensemble/cache/_read_overlay.json` (the
...
```
~/chatensemble/cache/<account>/2026-08.parquet   # every message, all channels
...
- `chatensemble cache` shows coverage; `chatensemble cache --clear [-a <account>]`
...
- **UI** - better output when loading accounts for first time, show message in case of error. Show "Loading cache from ~/chatensemble/cache/" when serving from disk. (Scan progress is done: `Fetching from channels [2/5]`, live, on startup and on `u`.)
- **Providers** — Discord (bot token, joined guilds only). Teams.


## Done
- **`import-slack`** — pulls the Slack web token (`xoxc-`) + `d` cookie (`xoxd-`)
  from a local Firefox profile into `~/chatensemble/accounts/<name>.yaml`; `--list`
  shows signed-in workspaces, `--workspace` picks one. Logic moved from the old
  `scripts/ff_slack_creds.py` into `chatensemble.slack_cookie` behind the subcommand;
  `cramjam` (for Slack's compressed config blob) is the `slack-cookie` extra.
- **Ask-Claude via the cache** — `chatensemble install-skill` ships a Claude Code
  skill (`~/.claude/skills/chatensemble/`) that queries the Parquet cache directly
  (DuckDB over `~/chatensemble/cache/*/*.parquet`), with the schema, the
...
- **Browser is the default command** — bare `chatensemble` fetches the real
...
- **Mark read** — `x` marks the selected thread read, `X` every thread in the tab;
  ...
  into a **local read-state overlay** (`~/chatensemble/cache/_read_overlay.json`,
  ...
  overlay with `chatensemble cache --clear`.
